...
over opId fab s = getId $ opId (Id . fab) s
...
toListOf opt s = appEndo (getK $ opt (\a -> K (Endo (a:))) s) []
...
preview opt s =  getFirst $ getK (opt (K . First . Just) s)
  ...
  -- preview and toListOf has the same structure!
...
_flip opt = lmap tflip $  rmap (fmap tflip) opt
  where tflip (a,b) = (b,a)
