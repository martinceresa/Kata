...
comp' :: SKI (b -> c) -> SKI (a -> b) -> SKI (a -> c)
comp' f g = comp <#> f <#> g

flip' :: SKI ((a -> b -> c) -> (b -> a -> c))
flip' = Ap (Ap S (Ap (Ap S (Ap K S)) (Ap (Ap S (Ap K K)) S))) (Ap K K)

icomp :: SKI ((a -> b) -> (b -> c) -> (a -> c))
icomp = Ap flip' comp
icomp' :: SKI (a -> b) -> SKI (b -> c) -> SKI (a -> c)
icomp' f g = icomp <#> f <#> g

rotr :: SKI (a -> (c -> a -> b) -> c -> b)
rotr = comp <#> (icomp <#> flip') <#> rev

rotv :: SKI (a -> b -> (a -> b -> c) -> c)
rotv = comp' (icomp <#> rev) (comp' icomp rev)
